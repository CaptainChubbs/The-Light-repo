...
if ($stmt = $con->prepare('SELECT nurseID, password FROM nurse WHERE email = ?')) {
    ...
    if ($stmt->num_rows > 0) {
        ...
        if ($stmt = $con->prepare('INSERT INTO client (first_name, last_name, email, phone_number, address, city, province, postal_code, password, qualifications, experience, transport, computer_skills, own_practice, practice_number, dispensing_number ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )')) {
